...
public function superAdmin(Request $request): JsonResponse
{
    ...
    // Single-pass tenant KPI derivation
  ...
  $latencyMs = round((microtime(true) - $startTime) * 1000, 1);
    ...
    // ── security ─────────────────────────────────────────────────────────
    // OPTIMIZED: resolveStoreIds() eliminates duplicate scoping code;
    // paginate with configurable per_page (default 3, max 100).
public function security(Request $request): JsonResponse
{
    $storeIds = $this->resolveStoreIds($request);
    $perPage  = max(1, min((int) ($request->per_page ?? 3), 100));

    $query = AuditLog::with('user:user_id,first_name,last_name')  // ← remove full_name
        ->latest('created_at');
    ...
    $events = $paginated->getCollection()->map(fn($log) => [
        ...
        'message'    => $log->action,
        'level'      => 'info',
        // Build full_name from first_name + last_name since it's not a DB column
    ...
    private function billingSelectColumns(string $today): array
    {
        return [
            ...
            // All unpaid balances (not date-filtered — balance lives on old billings)
            ...
            // Voids = drafts created today (no void status in this system)
            DB::raw("COUNT(CASE WHEN DATE(billing_date) = '{$today}' AND is_draft = 1 THEN 1 END) AS today_voids"),
            // today_outstanding placeholder — overridden by customerOutstanding in response
            DB::raw("SUM(CASE WHEN is_draft = 0 AND balance_due > 0 THEN balance_due ELSE 0 END) AS today_outstanding"),
        ];
    }

    /**
     * Accumulate all 11 billing aggregate columns in one Collection sweep.
     * Avoids eleven separate ->sum() calls each iterating the full collection.
     */
